...
public class ResultQuad extends AbstractResultElement {
	...
	protected void doComputations() {
		...
		// Berechne eine Ebene, die das Face beschreibt und teste, ob alle
		// Punkte in dieser Ebene liegen
		// durch Rundungsfehler kann der Fall auftreten, dass nicht alle Punkte
		// auf der Ebene liegen
		// allerdings ist dies nicht zwingend der Fall! Verzichte zunaechst auf
		// die Assertion
		// Plane plane = new Plane(mNormal, vert0.getPosition());
		// for(int i = 0; i < mPoints.size(); i++) assert
		// mMathHelper.isPointOnPlane(mPoints.get(i).getPosition(), plane):
		// "FEHLER: Punkt " + i + ": " + mPoints.get(i) +
		// " liegt nicht auf der Ebene des ResultQuads, dadurch wird das Quad nicht texturiert";
		...
		MyVector3f rayDirection = mMathHelper.calculateRotatedVectorRadians(
				mNormal, upperEdge, Math.PI / 2.0d);
		...
		// hier muss es nach Konstruktion einen Schnittpunkt geben...verwende
		// einen angenaeherten, falls kein direktes Ergebnis kommt
		...
		String message = "Es konnten keine Schnittpunkte mit der Grundkante ermittelt werden, SchnittpunktX3: "
				+ schnittpunktX3
				+ " SchnittpunktX2: "
				+ schnittpunktX2
				+ " Strahlen X3: Strahl1: "
	...
	// ------------------------------------------------------------------------------------------
	/**
	 * Methode testet, ob alle vier Punkte des Quads in der gleichen Ebene
	 * liegen. Ist dies nicht der Fall, versucht man, den Ausreisser in die
	 * errechnete Ebene zu projizieren.
	 * 
	 * @deprecated
	 */
	@Deprecated
	...
	public boolean validatePlane() {
		assert mPoints.size() > 2 : "FEHLER: Die Anzahl der gesetzten Punkte "
				+ mPoints.size() + " ist zu niedrig!";
		Vertex3d vert0 = mPoints.get(0);
		Vertex3d vert1 = mPoints.get(1);
		Vertex3d vert3 = mPoints.get(3);
		...
		while (pointIter.hasNext()) {
			...
			if (!mMathHelper.isPointOnPlane(currentPoint.getPosition(),
					testPlane)) {
				mMathHelper.projectPointOntoPlane(testPlane, currentPoint);
				assert mMathHelper.isPointOnPlane(currentPoint.getPosition(),
						testPlane) : "FEHLER: Punkt "
						+ currentPoint
						+ " befindet sich auch nach der Modifikation nicht auf der Zielebene!";
			}
			index++;
		}
