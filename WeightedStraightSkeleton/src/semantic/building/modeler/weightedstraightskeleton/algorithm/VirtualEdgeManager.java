...
public class VirtualEdgeManager {
	...
	/** Logger */
	protected static Logger LOGGER = Logger.getLogger(VirtualEdgeManager.class);
	...
	public VirtualEdge addVertsToVirtualEdge(SkeletonVertex start,
		...
		LOGGER.debug("Es wurde eine neue virtuelle Kante: " + newEdge
				+ " erstellt");
	...
	// ------------------------------------------------------------------------------------------
	/**
	 * Methode versucht, ein Twin-Vertex zu einer (oder mehreren)
	 * VirtualEdge-Datenstrukturen hinzuzufuegen
	 * 
	 * @param twin
	 *            Zwilling, das zu VirtualEdge-Strukturen geadded werden soll
	 * @return True, sofern es zu mindestens einer Kante hinzugefuegt wurde,
	 *         False sonst
	 */
	public boolean addTwinVertexToVirtualEdge(SkeletonVertex twin) {
		...
		// hole den Zwilling des Vertex, also das Originalvertex, da nur dieses
		// mit Sicherheit auf der Kante liegt
	 ...
	 * 
	 * @return Vector mit allen allen virtuellen Kanten in einer
	 *         Ray-Repraesentation
	 */
	...
	public void finalizeVirtualEdgeStructures() {
		...
		for (int i = 0; i < mVirtualEdges.size(); i++)
			mVirtualEdges.get(i).sortEdgeVerticesByDistance();
		// System.out.println("Insgesamt wurden " + mVirtualEdges.size() +
		// " VirtualEdge-Strukturen aufgebaut");
	}
	...
	public String toString() {
		...
		String result = "Insgesamt befinden sich " + mVirtualEdges.size()
				+ " virtuelle Kanten im Edge-Manager" + lineBreak;
		for (int i = 0; i < mVirtualEdges.size(); i++) {
	...
	public SkeletonVertex getNeighbourForVertex(SkeletonVertex start,
		...
		// wenn fuer die beiden Vertices keine virtuelle Kante bestimmt werden
		// konnte, gebe den Eingabenachbar wieder zurueck
		if (relevantEdge == null)
			return currentNeighbour;

		SkeletonVertex newNeighbour = relevantEdge.getNeighbourForVertex(start,
				currentNeighbour);
		// System.out.println("Angefragtes Vertex: " + start + " Nachbar: " +
		// currentNeighbour + " Rueckgabe: " + newNeighbour);
	...
	// ------------------------------------------------------------------------------------------
	/**
	 * Methode bestimmt einen neuen Nachbarn fuer den uebergebenen Nachbarn, der
	 * das uebergebene geloeschte Vertex ersetzt
	 * 
	 * @param neighbour
	 *            Vertex, fuer das ein neuer Nachbar bestimmt werden soll
	 * @param deletedVertex
	 *            Vertex, das der vorherige Nachbar war, aber geloescht wurde
	...
	public SkeletonVertex getNeighbourForDeletedVertex(
		...
		// wenn fuer die beiden Vertices keine virtuelle Kante bestimmt werden
		// konnte, versuche ein manuelles Update
		// weise dem Nachbarn mit Index i den Nachbarn mit Index i des
		// geloeschten Vertex zu
		if (relevantEdge == null) {
			...
			// gebe als Fallback das Vertex selber zurueck (dann gehen
			// wenigstens Edge-Updates nicht vollkommen schief)
			return neighbour;
		}

		SkeletonVertex newNeighbour = relevantEdge
				.getNeighbourForDeletedVertex(neighbour, deletedVertex);

		// System.out.println("Nachbarschaftsupdate fuer geloeschtes Vertex: " +
		// deletedVertex + " Nachbar: " + neighbour + " Rueckgabe: " +
		// newNeighbour );

		// bestimme den Nachbarn auf der Kante und gebe ihn an den Aufrufer
		// zurueck
		return newNeighbour;
	}
	...
	// ------------------------------------------------------------------------------------------
	/**
	 * Methode versucht, ein Vertex mit Kind zu finden, das zwischen start und
	 * validNeighbour auf einer virtuellen Kante liegt
	 * 
	 * @param start
	 *            Kinderloses Vertex, fuer das ein Ersatz bestimmt werden soll
	 * @param validNeighbour
	 *            Gueltiger Nachbar mit Kind
	 * @param isStart
	 *            Gibt an, ob es sich beim ersten Vertex um das Startvertex der
	 *            Kante handelt
	 * @return Vertex mit Kind auf einer virtuellen Kante zwischen start und
	 *         validNeighbour, null sonst
	 */
	public SkeletonVertex getNeighbourForVertexForResult(SkeletonVertex start,
		...
		// wenn keine virtuelle Kante fuer die Eingabevertices gefunden werden
		// konnte, gebe null zurueck
		if (relevantEdge == null)
			return null;

		// es konnte eine virtuelle Kante gefunden werden, versuche, ein Vertex
		// auf dieser Kante zu bestimmen
		SkeletonVertex newNeighbour = relevantEdge
				.getNeighbourForVertexForResult(start, validNeighbour, isStart);

		// System.out.println("Suche gueltigen Nachbar zwischen start: " + start
		// + " und end: " + validNeighbour + " Rueckgabe: " +newNeighbour);
		return newNeighbour;
	...
	// ------------------------------------------------------------------------------------------
	/**
	 * Methode durchlaeuft alle virtuellen Kanten im Kantenbuffer und sucht eine
	 * Kante, auf der die beiden uebergebenen Vertices liegen
	 * 
	 * @param vertex1
	 *            Eingabevertex1
	 * @param vertex2
	 *            Eingabevertex2
	 * @return Kante, auf der beide Vertices liegen, sofern eine solche gefunden
	 *         wurde, sonst null
	 */
	public VirtualEdge getEdgeForVertices(SkeletonVertex vertex1,
			SkeletonVertex vertex2) {
		VirtualEdge currentEdge = null;

		// finde zunaechst die virtuelle Kante, auf der sich die beiden Vertices
		// befinden
		Iterator<VirtualEdge> edgeIter = mVirtualEdges.iterator();
		while (edgeIter.hasNext()) {
			currentEdge = edgeIter.next();
	 ...
	 * 
	 * @param edges
	 *            Vector mit VirtualEdges, die zum aktuellen Manager
	 *            hinzugefuegt werden
	 * 
	 */
