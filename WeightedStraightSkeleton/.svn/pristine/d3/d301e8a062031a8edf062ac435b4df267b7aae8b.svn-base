...
/**
 * @author Patrick Gunia Erweiterung der Standard-Vertex-Klasse zur Verwaltung
 *         von zusaetzlich benoetigten Informationen fuer den
 *         Straight-Skeleton-Algorithmus WICHTIG: alle Getter fuer Vektoren
 *         geben Deep-Copies zurueck!
 */
public class SkeletonVertex extends Vertex3d {
	...
	/**
	 * Buffer der Nachbarverarbeitung, haelt die urspruenglich gesetzten
	 * Nachbarn vor den reflexiven Updates vor
	 */
	...
	/**
	 * Rotierte Winkelhalbierende, berechnet als Schnittgerade der adjazenten
	 * Planes am aktuellen Vertex
	 */
	...
	/**
	 * wenn das Vertex im Shrink-Prozess seine finale Position erreicht hat,
	 * wird es als Processed markiert und nicht weiter geschrumpft
	 */
	protected boolean mProcessed;

	/**
	 * Wird auf true gesetzt, wenn es sich um ein Vertex handelt, das auf dem
	 * Skelett liegt
	 */
	...
	/**
	 * Bei SplitEvents werden exakte Kopien erstellt, die in diesem vertex
	 * gespeichert werden
	 */
	protected SkeletonVertex mTwinVertex = null;

	/**
	 * Markiert, ob ein Vertex waehrend der Verarbeitung von Split- oder
	 * Vertex-Events bereits zu einem neuen Polygon hinzugefuegt wurde
	 */
	private boolean mAddedToNewPolygon = false;

	/**
	 * Bei Split- oder Vertex-Events markiert diese Variable, ob es sich um ein
	 * Original- oder ein Zwillingsvertex handelt
	 */
	private boolean mIsTwinVertex = false;

	/**
	 * Markiert, ob das Vertex im Zuge der Verarbeitung aus dem Child-Buffer
	 * geloescht wurde
	 */
	...
	public MyVector3f getNormal() {
		// assert mNormal != null:
		// "Normalenvektor ist null, Kopie kann nicht erzeugt werden";
		if (mNormal == null)
			return null;
	 ...
	 * 
	 * @param index
	 *            Index der gesuchten Kante im Kantenarray
	 ...
	 * 
	 * @param index
	 *            Index eds gesuchten Nachbarn innerhalb des Nachbararrays
	 * @return Nachbar mit dem uebergebenen Zielindex, Null, wenn der Zielindex
	 *         nicht definiert ist
	 */
	...
	// ------------------------------------------------------------------------------------------
	/**
	 * Methode legt ein Backup des Nachbararrays an, bevor dieses bidirektional
	 * aktualisiert wird.
	 */
	...
	// ------------------------------------------------------------------------------------------
	/**
	 * Nachdem die Verarbeitung auf der jweiligen Iteration abgeschlossen wurde,
	 * setzt man den Neighbour-Buffer zurueck
	 */
	...
	public boolean equals(Object obj) {
		 ...
		 * .floatToIntBits(other.mSplitVertexDistance)) return false;
		 */
	...
	/**
	 * Berechnet den auessersten Vorfahr des aktuell Knoten, also denjenigen
	 * Knoten im Graph, der keinen Elternknoten besitzt
	 * 
	...
	public void setNeighbourOnIndex(int index, SkeletonVertex neighbour) {
		if (hasNeighbourWithIndex(index)) {
			LOGGER.debug("SkeletonVertex.setNeighbourWithIndex: Ersetze Nachbar + "
					+ mNeighbours[index]
					+ "  mit Index "
					+ index
					+ " durch "
					+ neighbour + " in Vertex: " + this);
		}
	...
	public boolean hasNeighbours() {
		if (mNeighbours[0] != null && mNeighbours[1] != null)
			return true;
		else
			return false;
	}
	...
	// ------------------------------------------------------------------------------------------
	/**
	 * bekommt als Uebergabe ein Vertex uebergeben, sofern dieses bereits als
	 * Neighbour gesetzt wurde, gibt die Methode den Index des Neighbours
	 * zurueck, sonst -1
	 * 
	 * @param neighbour
	 *            SkeletonVertex, dessen Index innerhalb der Nachbarstruktur
	 *            gesucht werden soll
	 * @return Index innerhalb der Nachbarstrukturen, an dem sich das gesuchte
	 *         Vertex befindet, -1 sonst
	 */
	public int getIndexForNeighbour(SkeletonVertex neighbour) {

		for (int i = 0; i < mNeighbours.length; i++) {
			if (mNeighbours[i] != null) {
				if (mNeighbours[i] == neighbour)
					return i;
			}
		}
	...
	public int getIndexForNeighbourInBuffer(SkeletonVertex neighbour) {
		...
		for (int i = 0; i < mNeighbourBuffer.length; i++) {
			if (mNeighbourBuffer[i] != null) {
				if (mNeighbourBuffer[i] == neighbour)
					return i;
			}
		}
	...
	// ------------------------------------------------------------------------------------------
	/**
	 * Fuehrt eine clone-Operation der Komponenten durch, die nicht nur fuer den
	 * Algorithmus relevant sind (bsw. SplitPlanes, Bisectors etc.)
	 */
	...
	/**
	 * Clone-Methode fuer Skeleton-Vertices => aktuell nicht kopiert werden
	 * Kinder, Nachbarn und Nachbarstrahlen
	 */
	@Override
	public SkeletonVertex clone() {
		...
		if (hasParent())
			result.setParent(mParent);
		if (mWinkelhalbierende != null)
			result.setWinkelhalbierende(mWinkelhalbierende.clone());
		result.setIsReflexVertex(mbIsReflexVertex);
		if (mNormal != null)
			result.setNormal(getNormal());
		if (mRotatedWinkelhalbierende != null)
			result.setWinkelhalbierende(mRotatedWinkelhalbierende.clone());
		if (mOrtho != null)
	...
	public String toString() {
		...
		if (isTwinVertex())
			message += " Twin-Vertex";
		if (isReflexVertex())
			message += " Reflex-Vertex";
		return message;
	}

	// ------------------------------------------------------------------------------------------
	/**
	 * Methode liefert das unterste Kind in der Hierarchie (beruecksichtigt
	 * dabei aber keine Zwillingsvertices
	 * 
	...
	public SkeletonVertex getInnerMostChild() {
		...
		while (current.hasChild()) {
			...
			// sobald das Vertex einen Event ausgeloest hat, kann sich die
			// Richtung vollstaendig aendern
	...
	// ------------------------------------------------------------------------------------------
	/**
	 * Methode berechnet die Slope-Planes fuer die gespeicherten
	 * Skeleton-Edge-Kanten
	 * 
	 * @param algorithm
	 *            Wird fuer den Zugriff auf den SlopePlaneService benoetigt
	 */
	...
	public String printNeighbours() {
		...
		for (int i = 0; i < mNeighbours.length; i++) {
			if (mNeighbours[i] == null)
				continue;
			message += ".......................... " + i + ": "
					+ mNeighbours[i].toString() + lineSeparator;
		}
		return message;
