...
public class RayWrapper extends ModelSynthesisBaseGeometry {
	...
	/**
	 * ACHTUNG: Die gewrappten Vertices werden bei Skalierungen des
	 * Komponentenstrahls nicht skaliert!
	 */
	...
	/**
	 * Wenn es sich um einen parallelen Startstrahl handelt, ist hier der
	 * Quellstrahl gespeichert
	 */
	...
	public void addFaceUndirected(final Face face) {
		LOGGER.trace("Adde Face " + face.getID()
				+ " als ungerichteten Nachbarn zu Strahl " + getID());
		if (!mFaces.contains(face))
			mFaces.add(face);
	}
	...
	// ------------------------------------------------------------------------------------------
	/**
	 * hashCode() und equals() betrachten nur das Start- und Endvertex, um die
	 * Gleichheit der Strahlen zu bestimmen
	 */
	@Override
	public int hashCode() {
		final int prime = 31;
	...
	public Set<ModelSynthesisBaseGeometry> getAdjacentComponents() {
		Set<ModelSynthesisBaseGeometry> result = new HashSet<ModelSynthesisBaseGeometry>(
				2);
