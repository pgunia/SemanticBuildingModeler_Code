...
/**
 * @author Patrick Gunia Klasse zur Beschreibung eines Strahls durch eine
 *         3d-Vektor-Darstellung definiert durch Stuetzvektor und
 *         Richtungsvektor WICHTIG: alle Getter dieser Klasse geben Kopien
 *         zurueck, keine Pointer auf die eigentlichen Objekte!
 */
public class Ray implements Cloneable {
	...
	public Ray(final MyVector3f start, final MyVector3f direction) {
		...
		assert start != null && start != null : "FEHLER: Ein Eingabevektor ist null: Start: "
				+ mStart + " Direction: " + mDirection;
		mStart = start.clone();
	...
	public String toString() {
		return "Start: " + mStart + " End: " + mEnd + " Direction: "
				+ mDirection;
	}
	...
	// ------------------------------------------------------------------------------------------
	/**
	 * Methode skaliert den aktuellen Strahl um den uebergebenen
	 * Skalierungsfaktor
	 * 
	 * @param sclaeFactor
	 *            Skalierungsfaktor
	...
	public Double getXSlope() {
		...
		double angle = MyVectormath.getInstance().getFullAngleRad(
				positiveXAxis, getDirection());

		// bei einem Winkel von 0.0° koennen die Vektoren antiparallel sein =>
		// testen
		if (angle == 0.0d) {
			if (MyVectormath.getInstance().isAntiparallel(positiveXAxis,
					getDirection()))
				angle = Math.PI;
		}

		// bilde den Winkel auf den Bereich 0...Pi ab
		// dadurch spielt die Strahlenrichtung fuer die Labelbestimmung keine
		// Rolle
